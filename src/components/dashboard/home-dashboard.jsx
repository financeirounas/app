      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col gap-1">
          ...
          <span className={cn(
            ...
            isDoacao ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"
          )}>
            {type}
          </span>
        </div>
...
export default function HomeDashboard() {
  ...
  useEffect(() => {
    ...
    const loadMetrics = fetch('/api/reports/my-report')
      .then(res => {
      ...
      .then(data => {
        if (data.ok && data.report) {
          ...
          const avgAttendance = frequencies.length > 0
            ? Math.round(frequencies.reduce((sum, f) => sum + (f.amount || 0), 0) / frequencies.length)
            : 0;
          ...
          setMetrics({
            ...
            monthlyExpense: `R$ ${(report.metrics?.total_spending || 0).toFixed(2).replace('.', ',').replace(/\B(?=(\d{3})+(?!\d))/g, '.')}`,
            costPerCapita: `R$ ${(report.metrics?.cost_per_capita || 0).toFixed(2).replace('.', ',').replace(/\B(?=(\d{3})+(?!\d))/g, '.')}`,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-start justify-between mb-4">
            <span className="text-gray-600 text-base font-normal">Gasto Mensal</span>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-semibold text-gray-900">{metrics.monthlyExpense}</span>
          </div>
        </div>
        <MetricCard 
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-start justify-between mb-4">
            <span className="text-gray-600 text-base font-normal">Frequência Atual</span>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-semibold text-gray-900">{metrics.currentAttendance}</span>
          </div>
        </div>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">Você não possui nenhuma unidade cadastrada, verifique com o financeiro</p>
            </div>
          ) : stockItems.length === 0 ? (
            <div className="text-center py-8">
              <Package className="w-12 h-12 text-gray-300 mx-auto mb-2" />
              <p className="text-gray-600">Nenhum item em estoque</p>
              <p className="text-sm text-gray-500 mt-1">Comece adicionando itens ao estoque</p>
            </div>
          ) : (
